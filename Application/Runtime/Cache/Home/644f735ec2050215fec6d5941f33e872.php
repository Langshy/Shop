...
<header>
    <div class="header_nav nav-ab">
        ...
        <div class="centre">
            <ul>
                <li>
                    <a href="<?php echo U('Home/Index/index');?>" target="frameContent">首页</a>
                    <a href="<?php echo U('Home/Index/index');?>" target="frameContent">关于我们</a>
                    <a href="<?php echo U('Home/Shop/index');?>"  class="check-this">在线商城</a>
                    <a href="<?php echo U('Home/Index/index');?>" target="frameContent">联系我们</a>
                    <a href="<?php echo U('Home/Index/index');?>" target="frameContent">新闻动态</a>
                    <a href="#">下载app</a>
                </li>
            </ul>
        </div>
        <div class="right">
            ...
            <?php if(empty($userName) == true): ?><i class="icon-login getLogin user"></i>
...
<div class="type">
    <ul>
        <li><a href="<?php echo U('Home/Shop/part');?>?id=all">全部</a></li>
        <?php if(is_array($type)): $i = 0; $__LIST__ = $type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($i % 2 );++$i; if($t["tid"] == $tid): ?><li><a href="<?php echo U('Home/Shop/part');?>?id=<?php echo ($t["tid"]); ?>" class="type-this"><input type="hidden" value="<?php echo ($t["tid"]); ?>"><?php echo ($t["tname"]); ?></a></li>
                <?php else: ?>
                <li><a href="<?php echo U('Home/Shop/part');?>?id=<?php echo ($t["tid"]); ?>"><input type="hidden" value="<?php echo ($t["tid"]); ?>"><?php echo ($t["tname"]); ?></a></li><?php endif; endforeach; endif; else: echo "" ;endif; ?>
    </ul>
</div>
<div class="content">
    <div class="column">
        <a href="">在线商城</a><span> > </span>
        <?php if(is_array($type)): $i = 0; $__LIST__ = $type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($i % 2 );++$i; if($t["tid"] == $tid): ?><a href=""><?php echo ($t["tname"]); ?></a><?php endif; endforeach; endif; else: echo "" ;endif; ?>
    </a>
    </div>
    <div class="show-img"><img src="<?php echo ($product["photoaddress"]); ?>" width="606"/></div>
    <div class="sale">
        <ul>
            <input type="hidden" value="<?php echo ($product["pid"]); ?>" class="pid">
            <li class="sala-header"><?php echo ($product["pname"]); ?></li>
            <li class="sala-price">¥ <?php echo ($product["price"]); ?></li>
            <li class="sala-list">
                <span><img src="/Public/Home/images/形状11.png" height="20" width="20"/> 欧美标准质量&nbsp;&nbsp;</span>
                <span><img src="/Public/Home/images/形状11.png" height="20" width="20"/> 24小时快速发货&nbsp;&nbsp;</span>
                <span><img src="/Public/Home/images/形状11.png" height="20" width="20"/> 30天无忧退货</span>
            </li>
            <li class="sala-note"><p>尺      寸：L210×W65×H40cm</p>
                <p>重      量：58kg</p>
                <p>材      料：进口纯羊毛布料、特级白橡木</p></li>
            <li class="sale-color">颜色：
                <label class="radio"><input type="radio" name="radio" value="1" checked><i></i></label>
                <label class="radio"><input type="radio" name="radio" value="2" class="radio-two"><i></i></label>
                <label class="radio"><input type="radio" name="radio" value="3" class="radio-three"><i></i></label>
            </li>
            <li class="sale-num">数量：
                <input id="del" class="del" type="button" value="-"><input id="sale-num" class="input-num" type="text" value="1"><input id="add" class="add" type="button" value="+">
            </li>
            <li class="sale-select">
                <div class="getCart btn">加入购物车</div>
...
<div class="tail">
    ...
    <div class="tail-content">
        <ul>
            <li>
                <div class="tail-piece , left">
                    <p class="tail-title">Q：请问有现货吗？</p>
                    <p>
                    <div class="letter">A：</div>
                    <div class="new">大部分产品是没有现货的，正常发货时 间为45天，部分现货的产品有标注。</div>
                    </p>
                </div>
                <div class="tail-piece">
                    <p class="tail-title">Q：请问包邮上楼吗？</p>
                    <p>
                    <div class="letter">A：</div>
                    <div class="new">是的，顺丰包邮，除非您所居住的小区没有电梯且楼层在三层以上，会收取少量费用。</div>
                    </p>
                </div>
            </li>
            <li>
                <div class="tail-piece , left">
                    <p class="tail-title">Q：请问入门安装吗？</p>
                    <p>
                    <div class="letter">A：</div>
                    <div class="new">没有喔，需要自己动手搞定，随包有配 送工具，几分钟就ok。</div>
                    </p>
                </div>
                <div class="tail-piece">
                    <p class="tail-title">Q：请问是七天无理由退换货吗？</p>
                    <p>
                    <div class="letter">A：</div>
                    <div class="new">不是，非质量问题不接受七天无理由退 换。大件家具包装专业，拆装后无法确 保运回途中不受损伤，请谨慎下单。</div>
                    </p>
                </div>
            </li>
        </ul>
...
<script>
    $(function () {
        ...
        $('.getCart').click(function () {
            let user = $('.user');
            if(user.hasClass('icon-user')){
                let pid = $('.pid').val();
                let number = $('.input-num').val();
                $.ajax({
                    url:"<?php echo U('Home/Particulars/addCart');?>",
                    type:"POST",
